...
namespace winrt::Q2_2::implementation
{
    MainWindow::MainWindow()
    {
        ...
        {
            cp.Color(m_currentColor); // ColorPicker 초기값
        }
        UpdateColorPreview(); // 미리보기 업데이트

        if (auto tb = this->ThicknessValueText())
        {
            tb.Text(L"2.0 px"); // 굵기 초기값
        }
    }
    ...
    {
        // MainWindow -> IWindowNative -> HWND 추출
        if (auto native = this->try_as<::IWindowNative>())
        {
            ...
            if (SUCCEEDED(native->get_WindowHandle(&hwnd)) && hwnd)
            {
                ...
                {
                    (void)init->Initialize(hwnd); // HWND 전달
                }
            }
        }
    }
    ...
    {
        ContentDialog dlg{};
        dlg.XamlRoot(this->Content().XamlRoot());  // WinUI 3: XamlRoot 필수
    ...
    // ColorPicker 색상 변경 시 (드래그 중에는 미리보기만 갱신)
        ...
        Microsoft::UI::Xaml::Controls::Primitives::RangeBaseValueChangedEventArgs const& e)
    {
        ...
        {
            wchar_t buf[32]{};
            swprintf_s(buf, L"%.1f px", m_currentThickness); // 문자열 포맷
            tb.Text(buf);
        }

        if (m_currentStroke)
            m_currentStroke.StrokeThickness(m_currentThickness); // 현재 스트로크 적용
    }
...
// 새로 그리기 버튼 클릭
void winrt::Q2_2::implementation::MainWindow::NewButton_Click(winrt::Windows::Foundation::IInspectable const& sender,
